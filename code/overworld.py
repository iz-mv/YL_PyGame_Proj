...
class Icon(pygame.sprite.Sprite):
    def __init__(self, pos):
        super().__init__()
        self.pos = pos
        self.image = pygame.image.load('../graphics/overworld/select_level.png').convert_alpha()
        self.rect = self.image.get_rect(center=pos)
...
class Overworld:
    ...
    def input(self):
        ...
        if not self.moving and self.allow_input:
            if keyboard.is_pressed('d') and self.current_level < self.max_level:
                self.move_direction = self.get_movement_data('next')
                self.current_level += 1
                self.moving = True
            elif keyboard.is_pressed('a') and self.current_level > 0:
